...
fn solve(rock_lines: Vec<Vec<Vec2i>>, max_min: (Vec2i, Vec2i)) -> u32 {
    let width = max_min.0.x - max_min.1.x;
    let mut grid = vec![vec![Tile::AIR; (width + 3) as usize]; (max_min.0.y + 2) as usize];
    let offset = Vec2i::new(max_min.1.x - 1, 0);
    let source = Vec2i::new(500, 0) - offset;
    ...
    for rock_line in rock_lines {
        for i in 0..(rock_line.len() - 1) {
            ...
            let mut p = rock_line[i] - offset;
            let lim = rock_line[i+1] - offset;
            while p != lim {
