...
fn main() {
    ...
    println!("{:?}", target);
    let mut steps = a_star(&graph, (1, 0, 0), target, size);
    steps += a_star(&graph, (target.0, target.1, (steps as i16 % size.2)), (1, 0), size);
    steps += a_star(&graph, (1, 0, (steps as i16 % size.2)), target, size);

    println!("{:?}", steps);
    println!("Done in {}ms", start.elapsed().as_millis());
}

// dijkstra took almost 3 hours
