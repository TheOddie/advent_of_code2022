...
impl Monkey {
    ...
    fn next_item(&mut self) -> i32 {
        self.num_inspections += 1;
        self.items.pop().unwrap()
    }

    fn give_item(&mut self, item: i32) {
        self.items.push(item);
    }
}

fn solve(mut monkeys: Vec<Monkey>) -> i32 {
    ...
    for _ in 0..20 {
        ...
        for this_monkey_index in 0..len
        {
            ...
            while monkey.has_items()
            {
                ...
                let operand: Option<i32> = monkey.operand.parse::<i32>().ok();
    ...
    let mut counts = monkeys.iter().map(|m| m.num_inspections).collect::<Vec<i32>>();
    counts.sort();
    counts.reverse();
...
fn parse_input(s: &str) -> Vec<Monkey>
{
    ...
    for input in s.split("\n\n") {
        ...
        let starting_items = starting_items.split(", ").map(|x| x.parse::<i32>().unwrap()).collect();
